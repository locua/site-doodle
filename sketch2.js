...
let s1 = ( p ) =>{
    ...
    let num_objects=100,ip;

    p.setup = () => {
        p.createCanvas(p.windowWidth, p.windowHeight);
        num_objects=p.random(10);
    ...
    class Be {
        constructor(x,y,r,h) {
            ...
            this.y = y;
            this.r = r;
            this.incx=p.random(10);
            this.incy=p.random(10);
        ...
        draw() {
            ...
            p.fill(0);
            p.ellipse(this.x,this.y,this.r*p.noise(this.sinc)*0.1);
        }
        draw2(){
        ...
        update(){
            if(this.x>p.width-100)  this.x+=(0-p.noise(this.incx))*this.xscale;
            else if(this.x<100)   this.x+=(p.noise(this.incx))*this.xscale;
            else                  this.x+=(p.noise(this.incx)-0.5)*this.xscale;
            if(this.y>p.height-100) this.y+=(0-p.noise(this.incy))*this.yscale;
            else if(this.y<100)   this.y+=(p.noise(this.incy))*this.yscale;
